from . import _api
from flask import flash, render_template, Flask, request, jsonify
...
#customer dir#
@_api.route('/customer', methods=['GET', 'POST'])
def api_customer():
    ...
    if request.method == 'POST':
        ...
        if mongo.db.customer.insert_one(user_data):
...
#business dir#
@_api.route('/business', methods=['GET', 'POST'])
def api_business():
    if request.method == 'GET':
        return render_template('Signup.html')
    ...
    if request.method == 'POST':
        ...
        if mongo.db.business.insert_one(user_data):
            print('Username:', user_data['Username'])
            print('Password:', user_data['Password'])
            print('Email:', user_data['Email'])
            return render_template('ui.html', title='Business Login')
...
#user dir#
@_api.route('/user', methods=['GET', 'POST'])
def api_user():
    if request.method == 'GET':
        return render_template('Signup.html')
...
#business dir#
@_api.route('/ui/user/new/business', methods=['Get'])
def api_uibusiness():
...
#customer dir#
@_api.route('/ui/user/new/customer', methods=['Get'])
def api_uicustomer():
